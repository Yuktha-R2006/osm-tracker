      ...
      <aside className="hidden md:flex flex-col w-64 bg-[#1e293b] border-r border-slate-700">
        <div className="p-6">
          <h1 className="text-2xl font-bold gradient-text tracking-tight flex items-center gap-2">
            <MonitorPlay className="text-[#ff0055]" />
            OSM Tracker
          </h1>
        </div>
                ...
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `flex items-center gap-3 px-3 py-2 rounded-lg transition-colors ${
                      isActive ? 'bg-[#00f0ff]/20 text-[#00f0ff]' : 'hover:bg-slate-800 hover:text-white'
                    }`
                  }
                >
                  {link.icon}
            ...
            <>
              ...
              {navLinks.map((link) => (
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `flex items-center gap-3 px-3 py-2 rounded-lg transition-colors ${
                      isActive ? 'bg-[#ff0055]/20 text-[#ff0055]' : 'hover:bg-slate-800 hover:text-white'
                    }`
                  }
                >
                  {link.icon}
        ...
        <div className="p-4 border-t border-slate-700">
          <div className="flex items-center gap-3 px-3 py-2 mb-4 bg-slate-800/40 rounded-xl border border-white/5">
            <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-[#ff0055] to-[#00f0ff] flex items-center justify-center font-bold text-slate-900 shadow-md">
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        ...
        <header className="md:hidden flex items-center justify-between p-4 bg-[#1e293b] border-b border-slate-700 relative z-40">
          <h1 className="text-xl font-bold gradient-text flex items-center gap-2">
            <MonitorPlay size={24} className="text-[#ff0055]" />
            OSM
          </h1>
          <div className="flex items-center gap-4">
            <div className="relative">
              <button onClick={() => setNotifOpen(!notifOpen)} className="text-slate-400 hover:text-white">
                <Bell size={24} />
                {unreadCount > 0 && (
                  <span className="absolute -top-1 -right-1 w-3.5 h-3.5 bg-[#ff0055] rounded-full border-2 border-[#1e293b] flex items-center justify-center text-[8px] font-bold text-white">
                      ...
                      <NavLink
                        ...
                        className={({ isActive }) =>
                          `flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-300 ${
                            isActive ? 'bg-[#00f0ff]/20 text-[#00f0ff] shadow-[0_0_10px_rgba(0,240,255,0.3)]' : 'hover:bg-slate-800/60 hover:text-white'
                          }`
                        }
                      >
                        {link.icon}
                  ...
                  <>
                    ...
                    {navLinks.map((link) => (
                      <NavLink
                        ...
                        className={({ isActive }) =>
                          `flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-300 ${
                            isActive ? 'bg-[#ff0055]/20 text-[#ff0055] shadow-[0_0_10px_rgba(255,0,85,0.3)]' : 'hover:bg-slate-800/60 hover:text-white'
                          }`
                        }
                      >
                        {link.icon}
          ...
          <div className="flex-1 max-w-md">
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-slate-900/60 backdrop-blur-sm border border-slate-700 rounded-xl py-2.5 pl-10 pr-4 text-white placeholder-slate-500 focus:outline-none focus:border-[#00f0ff] focus:shadow-[0_0_15px_rgba(0,240,255,0.3)] transition-all duration-300"
                value={searchQuery}
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative">
              ...
              >
                <Bell size={20} />
                {unreadCount > 0 && (
                  <span className="absolute -top-1 -right-1 w-4 h-4 bg-[#ff0055] rounded-full border-2 border-[#0f172a] flex items-center justify-center text-[9px] font-bold text-white">
                    {unreadCount > 9 ? '9+' : unreadCount}
                  </span>
                )}
              </button>
            ...
            <div className="relative">
              ...
              >
                <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-[#ff0055] to-[#00f0ff] flex items-center justify-center text-sm font-bold text-slate-900 shadow-md">
                  {getInitials(user?.name)}
                </div>
                <div className="hidden lg:flex flex-col text-left">
                  <span className="text-xs font-semibold text-white leading-tight">{user?.name}</span>
                  <span className="text-[10px] text-[#00f0ff] font-medium leading-none">
        ...
        <div className="absolute top-[-10%] right-[-5%] w-96 h-96 bg-[#ff0055]/10 rounded-full blur-3xl pointer-events-none"></div>
        <div className="absolute bottom-[-10%] left-[-5%] w-96 h-96 bg-[#00f0ff]/10 rounded-full blur-3xl pointer-events-none"></div>
      </main>
    </div>
  );
};
